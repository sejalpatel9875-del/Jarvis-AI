...
_AGENT_CATALOG = [
    {"id": "ceo", "name": "CEO Agent", "icon": "👑", "role": "Executive decisions, KPIs and business recommendations", "capability": "Executive dashboard"},
    {"id": "planner", "name": "Planner Agent", "icon": "📋", "role": "Goals, task breakdowns and execution plans", "capability": "Autonomous planning"},
    {"id": "reasoning", "name": "Reasoning Agent", "icon": "🧠", "role": "Complex, multi-step decision analysis", "capability": "Structured reasoning"},
    {"id": "validator", "name": "Validator Agent", "icon": "✅", "role": "Output checks, risks and verification", "capability": "Quality validation"},
    {"id": "executor", "name": "Executor Agent", "icon": "⚡", "role": "Creates executable task plans and workflow handoffs", "capability": "Task execution"},
    {"id": "knowledge", "name": "Knowledge Agent", "icon": "📚", "role": "RAG document and knowledge-base questions", "capability": "Semantic retrieval"},
    {"id": "crm", "name": "CRM Agent", "icon": "💼", "role": "Lead, contact and opportunity intelligence", "capability": "CRM pipeline"},
    {"id": "sales", "name": "Sales AI Agent", "icon": "💰", "role": "Follow-ups, lead scoring and sales suggestions", "capability": "Sales assistance"},
    {"id": "automation", "name": "Automation Agent", "icon": "⚙️", "role": "Workflow design and automation guidance", "capability": "Workflow engine"},
    {"id": "search", "name": "Search Agent", "icon": "🔍", "role": "Searches workspace leads, deals, activity and workflows", "capability": "Global search"},
    {"id": "memory", "name": "Memory Agent", "icon": "🧠", "role": "Preferences and conversation context", "capability": "Persistent memory"},
    {"id": "activity", "name": "Activity Feed Agent", "icon": "📢", "role": "Workspace events and operational timeline", "capability": "Activity feed"},
    {"id": "notification", "name": "Notification Agent", "icon": "🔔", "role": "Dashboard alerts and reminders", "capability": "Notifications"},
    {"id": "dashboard", "name": "Dashboard Agent", "icon": "📊", "role": "CEO metrics, pipeline and system health", "capability": "Business dashboard"},
    {"id": "workspace", "name": "Workspace Agent", "icon": "🏢", "role": "Organizations and workspace isolation", "capability": "Workspace management"},
    {"id": "authentication", "name": "Authentication Agent", "icon": "🔐", "role": "JWT sessions and secure access status", "capability": "Authentication"},
    {"id": "rbac", "name": "RBAC Agent", "icon": "👥", "role": "Role and permission evaluation", "capability": "Access control"},
    {"id": "security", "name": "Security Agent", "icon": "🛡️", "role": "Security controls and request protections", "capability": "Security posture"},
    {"id": "rate-limiter", "name": "Rate Limiter Agent", "icon": "🚫", "role": "Brute-force protection controls", "capability": "Rate limiting"},
    {"id": "database", "name": "Database Agent", "icon": "🗄️", "role": "Workspace data-store readiness", "capability": "SQLite adapter"},
    {"id": "cache", "name": "Redis Cache Agent", "icon": "⚡", "role": "Fast retrieval with safe local fallback", "capability": "Cache layer"},
    {"id": "commands", "name": "Command Palette Agent", "icon": "⌨️", "role": "Keyboard shortcuts and global actions", "capability": "Command palette"},
    {"id": "onboarding", "name": "Onboarding Agent", "icon": "🎯", "role": "First-workspace setup progress", "capability": "Onboarding"},
]
...
def execute_agent(request: AgentExecutionRequest):
    ...
    if not prompt:
        raise HTTPException(status_code=400, detail="Tell the selected agent what you want it to do.")
    ...
    try:
        if agent_id in {"ceo", "dashboard"}:
            ...
            output = ask_ai(f"You are the CEO Agent. Reply in the same primary language and script as the user. Review these live workspace metrics and answer the request concisely with recommendations. Metrics: {result}\nRequest: {prompt}")
        elif agent_id == "knowledge":
            result = knowledge_engine.query_workspace_knowledge(workspace_id, prompt, 5)
            output = result.get("formatted_answer") or result.get("answer") or "No matching knowledge found."
            ...
            output = "Recent activity:\n" + ("\n".join(f"• {item.get('action')}: {item.get('details')}" for item in result) or "No activity recorded yet.")
        elif agent_id == "notification":
            result = notification_service.send_notification("default_user", "JARVIS agent request", prompt, "dashboard")
            output = "Dashboard notification created." if result.get("success") else result.get("error", "Could not create notification.")
        elif agent_id == "memory":
            learned = memory_agent.auto_learn_from_input(prompt)
            output = memory_agent.get_memory_context() + (f"\nNew facts learned: {learned}" if learned else "\nNo new preference pattern detected; current context is ready.")
        elif agent_id == "automation":
            workflows = automation_engine.list_workflows(workspace_id)
            output = ask_ai(f"You are the Automation Agent. Reply in the same primary language and script as the user. Design a safe no-code workflow for: {prompt}\nExisting workflows: {workflows}. Do not claim external actions were sent unless listed as configured.")
        elif agent_id == "crm":
            leads = crm_engine.list_leads(workspace_id)
            output = ask_ai(f"You are the CRM Agent. Reply in the same primary language and script as the user. Use this live lead data to answer: {prompt}\nLeads: {leads}")
        elif agent_id == "sales":
            leads = crm_engine.list_leads(workspace_id)
            output = ask_ai(f"You are the Sales AI Agent. Reply in the same primary language and script as the user. Draft practical, compliant sales guidance for: {prompt}\nLead data: {leads}")
            ...
            output = "Available commands:\n" + "\n".join(f"• {item['shortcut']} — {item['title']}" for item in commands)
            ...
            roles = {"planner": "goal planning and ordered execution steps", "reasoning": "structured multi-step reasoning", "validator": "quality validation and risk detection"}
            output = ask_ai(f"You are the JARVIS {roles[agent_id]} agent. Reply in the same primary language and script as the user. For the request below, give useful work product without revealing private chain-of-thought. Request: {prompt}")
        activity_feed.log_activity(workspace_id, f"{agent_id.title()} Agent", "agent_request_completed", prompt[:240], "AI_AGENT")
...
async def transcribe_voice_command(file: UploadFile = File(...)):
    ...
    if len(audio) > 12 * 1024 * 1024:
        raise HTTPException(status_code=413, detail="Voice recording is too large. Please keep it under 12 MB.")
    try:
        text = voice_pipeline.transcribe_audio(audio, file.filename or "voice.webm", file.content_type or "audio/webm")
        if not text:
            raise HTTPException(status_code=422, detail="I could not understand that recording. Please try again.")
        return {"text": text}
...
def refresh_auth_token(request: RefreshTokenRequest):
    ...
    new_tokens = auth_service.create_tokens(payload.get("sub"), payload.get("email"), payload.get("role", "user"))
    return new_tokens
