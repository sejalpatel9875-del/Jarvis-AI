...
from memory.manager import _default_manager, get_preference, get_recent, save_preference, save_turn
...
class TestV48MemoryUpgrades(unittest.TestCase):
    ...
    def test_long_term_memory(self):
        ...
        # Semantic recall
        search_res = _default_manager.semantic_search_long_term(
            "voice language conversational preference"
        )
        self.assertGreaterEqual(len(search_res), 1)
        self.assertTrue(any(mem_key == item["key"] for item in search_res))
        self.assertTrue(any(summary == item["abstract_summary"] for item in search_res))
