...
async function api(url,options={}){
  if(url==='/api/v1/agents'&&_apiCache.has(url))return _apiCache.get(url);
  const r=await fetch(url,options);
  if(r.status===401){location.assign('/');throw Error('Session ended.')}
  const d=await r.json();
  if(!r.ok)throw Error(d.detail||d.error||'Request failed.');
  if(url==='/api/v1/agents')_apiCache.set(url,d);
  return d;
}
...
async function loadWorkflows(){try{const d=await api(`/api/v1/workflows/history?workspace_id=${workspace}`),items=d.history||[];$('#metric-workflows').textContent=items.length;$('#workflow-history').innerHTML=items.length?items.slice(0,15).map(x=>`<article><b>${esc(x.status)}</b><span>Workflow #${esc(x.workflow_id)} · ${esc(x.executed_at)}</span></article>`).join(''):'<span class="empty">No workflow runs yet. Create a workflow to begin.</span>'}catch(e){$('#workflow-history').textContent=e.message}}$('#refresh-workflows').onclick=loadWorkflows;
...
async function renderHistory(){const list=$('#history-list');list.innerHTML='<span class="empty">Loading JARVIS conversation history…</span>';try{const d=await api('/api/v1/conversations?limit=15');const turns=d.conversations||[];if(turns.length){list.innerHTML=turns.slice(0,15).reverse().map(x=>`<article><b>You</b><span>${esc(x.timestamp)}</span><p>${rich(x.user_message)}</p><b>JARVIS</b><span>${esc(x.provider||'Hybrid AI')}</span><p>${rich(x.assistant_reply)}</p></article>`).join('');return}}catch(e){}const items=history().slice().reverse();list.innerHTML=items.length?items.slice(0,15).map(x=>`<article><b>${x.role==='user'?'You':'JARVIS'}</b><span>${esc(x.at)}</span><p>${rich(x.text)}</p></article>`).join(''):'<span class="empty">No conversations have been saved yet.</span>'}$('#clear-history').textContent='Clear local copy';
